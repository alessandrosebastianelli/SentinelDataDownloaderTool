...
def download(points, patch_size, start_date, end_date, date_names, s2_selectors, s1_selectors, n_of_regions, n_imgs, downloads_folder_path, download_path, sen2_images_base_path, sen1_images_base_path, windows, gui = False):
    regions, rectangles = gee.get_region_and_rectangle(points[0,:], points[1,:], size = patch_size)
    zone_names = get_zone_names(points)

    if gui:
        from progress_bar import progress_bar
        pb_scene =  progress_bar('Scenes downloading...', n_of_regions)
