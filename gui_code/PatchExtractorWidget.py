...
class PatchExtractorWidget:
  def __init__(self, root):
    ...
    self.extractor_x_size = tk.StringVar(root, value='256')
    self.extractor_y_size = tk.StringVar(root, value='256')
    self.createGUI(self.root)

  def createGUI(self, root):
    #--------------- PATCH EXTRACTOR FRAME ---------------------
    extractor_frame = tk.Frame(root)
    extractor_frame.grid(row=5, column=1, sticky=tk.E, pady=15)
    ...
    self.extractor_button = tk.Button(extractor_sub_frame_2, text='EXTRACT', height=1, width=15, font=self.helv16, command = None)
