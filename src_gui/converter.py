...
def find(directory, satellite, s2_selectors, s1_selectors):
    for region in glob(directory):        
        r = os.path.join(region, '*')
        for period in glob(r):
            p = os.path.join(period, '*')
            for scene in glob(p):
                base_name = os.path.join(scene, basename(scene))
...
def convert_s2(progressbar, s2_path, s1_path, image_size, s2_selectors, s1_selectors, s2_conversion_mode, windows):
    gen = iter(find(s2_path, 'sen2', s2_selectors, s1_selectors))
    s2_len = len(list(find(s2_path, 'sen2', s2_selectors, s1_selectors)))//4
    master_counter = 0

    progressbar['maximum'] = s2_len
...
def convert_s1(progressbar, s2_path, s1_path, image_size, s2_selectors, s1_selectors, s2_conversion_mode, windows):
    gen_2 = iter(find(s1_path, 'sen1', s2_selectors, s1_selectors))
    s1_len = len(list(find(s1_path, 'sen1', s2_selectors, s1_selectors)))
    counter = 0

    progressbar['maximum'] = s1_len
